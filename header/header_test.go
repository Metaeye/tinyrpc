...
// TestResponseHeader_GetFormat .
func TestResponseHeader_GetFormat(t *testing.T) {
	header := &ResponseHeader{
		Format:      0,
		Error:       "error",
		ID:          12455,
	...
	assert.Equal(t, true, reflect.DeepEqual(compressor.Format(0), header.GetFormat()))
}

// TestRequestHeader_GetFormat .
func TestRequestHeader_GetFormat(t *testing.T) {
	header := &RequestHeader{
		Format:     0,
		Method:     "Add",
		ID:         12455,
